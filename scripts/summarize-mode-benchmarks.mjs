...
console.log(JSON.stringify(reports.map(r=>({time:r.time,mode:r.mode,preview:r.preview,seconds:r.durationSeconds,ok:r.ok,message:r.message,timings:r.timings,checks:r.windows.filter(w=>w.finalSideWindowsVerified||w.finalTelegramPairVisible||w.distinctWallpapers||w.splitView||w.musicPlaying!==undefined||w.officeLighting||w.officeStart||w.centerForeground||w.learningERPMinimized||w.erpVisibleAtFinish||w.reusedTelegramPair||w.benchmarkHostPreserved||w.wallpaper).map(w=>w.wallpaper?{display:w.display,wallpaper:w.wallpaper.split('/').at(-1)}:w)})),null,2));
